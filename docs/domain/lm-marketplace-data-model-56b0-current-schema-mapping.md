...
| 12 | Buyer / session identity | src/lib/schema.ts, src/app/actions.ts | orders.sessionHash (varchar(64)), cartItems.sessionHash (varchar(64)), getSessionHash | Session hash cookie (session_hash, httpOnly, sameSite:lax, 30-day maxAge) used to identify buyer. No authenticated user identity; no B2B/B2C context captured. | BuyerIdentityReference, EcommerceBuyerLegalContextSnapshot, RfqBuyerLegalContextSnapshot | EXTENSION_CANDIDATE | VERIFIED_CURRENT_FACT | High | High | Authenticated identity and legal context required for B2B compliance. |
