...
| 08 | RFQ flow — action | src/app/actions.ts | submitRfq | Server action. Uses validatePublicRfqEligibility() and resolveCanonicalOfferModel() to require canonical RFQ eligibility (rfq + inbound). Inserts rfqLeads row (bigserial PK, offerId bigint, partnerId bigint). Returns {ok:true, code:"RFQ_SENT"}. No MarketplaceOrder, SellerOrder or PaymentOrchestration created. | RfqRequest | EXTENSION_CANDIDATE | VERIFIED_CURRENT_FACT | Medium | High | Canonical RFQ model is now strictly enforced. Must align with RfqRequest aggregate in future; current behavior preserved per DEC-MKT-18. |
| 09 | Cart | src/app/actions.ts, src/lib/schema.ts | addToCart, cartItems | addToCart fetches offer by id. Uses validateEcommerceEligibility() and resolveCanonicalOfferModel() to require CANONICAL_OFFER_MODEL=ecommerce. Inserts or updates cartItems (bigserial PK, offerId bigint, sessionHash varchar(64)). | MarketplaceOrder | EXTENSION_CANDIDATE | VERIFIED_CURRENT_FACT | High | High | Canonical ecommerce model is now strictly enforced; session-only buyer identity. Behavior preserved per DEC-MKT-18. |
| 10 | Checkout / orders | src/app/actions.ts, src/lib/schema.ts | submitCheckout, orders | submitCheckout inserts one orders row (bigserial PK, sessionHash varchar(64), status varchar(20) default 'new', totalAmount numeric). Revalidates cart items against database to compute final price. No seller-specific split. Clears cart. | MarketplaceOrder | EXTENSION_CANDIDATE | VERIFIED_CURRENT_FACT | High | High | Must support multi-seller split and immutable commercial snapshots. Behavior preserved per DEC-MKT-18. |
...
| 14 | clicks table | src/lib/schema.ts | clicks (bigserial PK, offerId bigint, partnerId bigint, clickedAt timestamp, sessionHash varchar(64), ipHash varchar(64), isUnique24h boolean) | Table exists with tracking fields. Has offerId, partnerId, sessionHash, ipHash, isUnique24h. | OutboundRedirectEvent, OutboundRedirectAuditLog | EXTENSION_CANDIDATE | VERIFIED_CURRENT_FACT | Medium | Medium | Click tracking is wired via recordOutboundClick using parameterized SQL. Residual debt: potential click int4 vs bigint mismatch. |
| 15 | /go/[id] route behavior | src/app/go/[id]/route.ts | /go/[id] route | Parses canonical positive offer id; fetches offer; requires isActive=true and conversion-allowed publication state; requires resolveCanonicalOfferModel(offerModel, conversionType) === "outbound"; validates curated HTTP/HTTPS outbound URL; preserves /go/[id] as external navigation path. When tracking config is valid, derives sessionHash and HMAC ipHash, schedules recordOutboundClick() via Next.js after(). Redirects with HTTP 302. | OutboundRedirectEvent, ExternalRedirectReference | EXTENSION_CANDIDATE | VERIFIED_CURRENT_FACT | High | Medium | Enforces CANONICAL_OFFER_MODEL=outbound. Click tracking is asynchronous. |
