...
| 04 | contractModel absence | src/lib/schema.ts | (none) | No contractModel field exists in offers or any other table. Logical OfferContractClassification concept has no physical representation. | OfferContractClassification | NO_CURRENT_ELEMENT | VERIFIED_ABSENCE | N/A | High | New field required for physical implementation. Blocked pending logical review. |
| 05 | Partner relationship | src/lib/schema.ts, src/app/actions.ts | offers.partnerId (bigint, NOT NULL); leftJoin partners on offers.partnerId = partners.id | Application-level join; no database FK declared. partners.id is BIGSERIAL PK. partnerId not validated as immutable at DB level. | OfferSellerAssignment | EXTENSION_CANDIDATE | VERIFIED_CURRENT_FACT | Medium | Medium | Database FK declaration and snapshot-capture on order creation required. |
...
| 12 | Buyer / session identity | src/lib/schema.ts, src/app/actions.ts | orders.sessionHash (varchar(64)), cartItems.sessionHash (varchar(64)), getSessionHash | Session hash cookie (session_hash, httpOnly, sameSite:lax, 30-day maxAge) used to identify buyer. No authenticated user identity; no B2B/B2C context captured. | BuyerIdentityReference, BuyerLegalContextSnapshot | EXTENSION_CANDIDATE | VERIFIED_CURRENT_FACT | High | High | Authenticated identity and legal context required for B2B compliance. |
...
| 14 | clicks table | src/lib/schema.ts | clicks (bigserial PK, offerId bigint, partnerId bigint, clickedAt timestamp, sessionHash varchar(64), ipHash varchar(64), isUnique24h boolean) | Table exists with tracking fields. Has offerId, partnerId, sessionHash, ipHash, isUnique24h. | OutboundRedirectEvent | REQUIRES_FURTHER_AUDIT | REQUIRES_FURTHER_AUDIT | Medium | Medium | /go/[id] does NOT insert into clicks (see row 15). Table ready for tracking but not currently wired. |
...
| 16 | Audit support | src/lib/schema.ts | createdAt, updatedAt timestamps on some tables | Timestamp columns only. No formal domain audit log table. | DomainAuditEvent | NO_CURRENT_ELEMENT | VERIFIED_ABSENCE | N/A | Medium | New audit journal required for domain events. |
| 17 | Idempotency support | (none) | (none) | No idempotency registry exists in schema or application code. | IdempotencyRecord | NO_CURRENT_ELEMENT | VERIFIED_ABSENCE | N/A | Low | New requirement; needed for safe retry and deduplication. |
...
| 26 | Chargeback structures | (none) | (none) | No ChargebackDispute exists. | ChargebackDispute | NO_CURRENT_ELEMENT | VERIFIED_ABSENCE | N/A | Low | New requirement; blocked on OMQ-MKT-09. |
