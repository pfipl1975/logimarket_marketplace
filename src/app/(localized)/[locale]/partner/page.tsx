...
export default async function LocalizedPartnerEntryPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  if (memberships.length === 1) {
    redirect(`/${locale}/partner/${memberships[0].partnerId}`);
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[50vh] p-8">
      ...
      <div className="grid gap-4 w-full max-w-md">
        {memberships.map((m) => (
          <a key={m.partnerId} href={`/${locale}/partner/${m.partnerId}`} className="block p-4 border border-border-industrial rounded-industrial hover:border-brand-teal transition-colors text-center font-medium text-brand-navy">
            {m.companyName}
          </a>
        ))}
      </div>
