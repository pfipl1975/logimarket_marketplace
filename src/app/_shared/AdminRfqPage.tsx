...
import { getAdminRfqPage } from "@/app/actions";
import { buildAdminRfqUrl } from "@/lib/admin/rfq-query";
import { AdminRfqTable } from "@/components/admin/AdminRfqTable";
import Link from "next/link";
import { Search } from "lucide-react";

export async function AdminRfqPage({
  locale,
  searchParams,
}: {
  locale: Locale;
  ...
  const result = await getAdminRfqPage(searchParams);
            ...
            {pageCount > 1 && (
              <div className="px-6 py-4 border-t border-border-industrial bg-brand-light-gray/30 flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  {query.page > 1 ? (
                    <Link
                      href={buildAdminRfqUrl(basePath, { page: query.page - 1 }, query)}
