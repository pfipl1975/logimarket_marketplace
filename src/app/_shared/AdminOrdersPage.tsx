...
import { getAdminOrdersPage } from "@/app/actions";
import { buildAdminOrdersUrl } from "@/lib/admin/orders-query";
...
export async function AdminOrdersPage({
  ...
  const dict = dictionary.adminOrders;

  const result = await getAdminOrdersPage(searchParams);
  ...
  const { items, totalCount, totalPages, query } = result.data;
