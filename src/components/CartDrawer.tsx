          ...
          {!hasItems ? (
            <div className="flex flex-1 flex-col items-center justify-center gap-4 py-12 text-center">
              <div className="flex h-16 w-16 items-center justify-center rounded-full bg-gray-100">
                <Package className="h-8 w-8 text-[#5a6472]" />
              </div>
              <div>
            ...
            <>
              <div className="flex-1 overflow-y-auto space-y-4 pr-1">
                {items.map((item) => (
                  <div key={item.id} className="flex gap-3 rounded-lg border bg-white p-3 border-[#d9dde2]">
                    <div className="h-20 w-20 shrink-0 overflow-hidden rounded-md bg-gray-100 relative">
                      {item.imageUrl ? (
                        ...
                        <div className="flex h-full w-full items-center justify-center">
                          <Package className="h-6 w-6 text-[#5a6472]" />
                        </div>
                      )}
                    </div>
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <div className="flex items-center gap-1">
                          <button onClick={() => item.quantity > 1 ? updateQuantity(item.id, item.quantity - 1) : removeFromCart(item.id)}
                            className="flex h-7 w-7 items-center justify-center rounded border hover:bg-gray-100 transition-colors border-[#d9dde2]">
              ...
              <div className="border-t pt-4 space-y-4 border-[#d9dde2]">
