...
export function RfqDialog({ offerId, offerTitle, partnerName, className }: {
  offerId: number; offerTitle: string; partnerName: string; className?: string;
}) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className={`gap-2 font-semibold text-white border-0 ${className ?? ""}`}
          ...
          <FileText className="h-4 w-4" />Zapytaj o wycenę
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-md">
        {success ? (
          <div className="flex flex-col items-center gap-3 py-8 text-center">
            ...
            <p className="text-base font-medium">Zapytanie wysłane pomyślnie!</p>
            <p className="text-sm text-muted-foreground">Partner przygotuje wycenę B2B i skontaktuje się z Tobą.</p>
          </div>
        ) : (
          <>
            <DialogHeader>
              <DialogTitle className="text-lg font-bold">Zapytanie o wycenę (RFQ)</DialogTitle>
              <DialogDescription>
                <span className="font-semibold" style={{ color: "#141c2c" }}>{offerTitle}</span>{" — "}{partnerName}. Zapytanie trafi bezpośrednio do partnera.
              </DialogDescription>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="grid gap-4 mt-2">
              <div className="grid gap-1.5"><Label htmlFor="rfq-company">Nazwa firmy *</Label>
                <Input id="rfq-company" value={formData.companyName} onChange={(e) => setFormData({ ...formData, companyName: e.target.value })} required /></div>
              <div className="grid gap-1.5"><Label htmlFor="rfq-contact">Osoba kontaktowa *</Label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="grid gap-1.5"><Label htmlFor="rfq-email">E-mail *</Label>
                  <Input id="rfq-email" type="email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} required /></div>
                <div className="grid gap-1.5"><Label htmlFor="rfq-phone">Telefon</Label>
                  <Input id="rfq-phone" type="tel" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} /></div>
              </div>
              <div className="grid gap-1.5"><Label htmlFor="rfq-msg">Wiadomość</Label>
                <Textarea id="rfq-msg" rows={3} placeholder="Liczba sztuk, termin, dodatkowe wymagania..."
              ...
              <Button type="submit" disabled={pending} className="w-full font-semibold gap-2 text-white border-0"
                ...
                {pending ? <><Loader2 className="h-4 w-4 animate-spin" />Wysyłanie...</> : "Wyślij zapytanie"}
              </Button>
              <p className="text-center text-xs text-muted-foreground">Wysyłając formularz wyrażasz zgodę na przekazanie danych partnerowi.</p>
            </form>
          </>
        )}
      </DialogContent>
