...
export type AdminOfferPublicationTarget = "published" | "archived" | "draft";
...
export type AdminOfferPublicationResult =
  | {
      ok: true;
      code: "OFFER_PUBLISHED" | "OFFER_ARCHIVED" | "OFFER_RETURNED_TO_DRAFT";
      changed: boolean;
    }
  | {
      ok: false;
...
export function parseAdminOfferPublicationInput(rawInput: unknown): AdminOfferPublicationInput | null {
  ...
  const validExpected = ["draft", "pending_review", "published", "archived", "hidden", "deleted"];
  const validTarget = ["published", "archived", "draft"];
...
export type TransitionResult =
  ...
  | { kind: "PROCEED_RETURN_TO_DRAFT" }
  | { kind: "IDEMPOTENT_DRAFT" }
...
export function evaluateOfferPublicationTransition(
  ...
  if (targetStatus !== "published" && targetStatus !== "archived" && targetStatus !== "draft") {
    return { kind: "INVALID_TRANSITION" };
  }

  // Same-state / idempotency
  if (currentStatus === targetStatus) {
    if (targetStatus === "published") return { kind: "IDEMPOTENT_PUBLISHED" };
    if (targetStatus === "archived") return { kind: "IDEMPOTENT_ARCHIVED" };
    if (targetStatus === "draft") return { kind: "IDEMPOTENT_DRAFT" };
    return { kind: "INVALID_TRANSITION" };
  }
  ...
  // Allowed transitions
  if ((currentStatus === "draft" || currentStatus === "pending_review") && targetStatus === "published") {
  ...
  if (currentStatus === "pending_review" && targetStatus === "draft") {
    return { kind: "PROCEED_RETURN_TO_DRAFT" };
  }
  ...
  try {
    return await db.transaction(async (tx) => {
      ...
      switch (decision.kind) {
        ...
        case "IDEMPOTENT_DRAFT":
          return { ok: true, code: "OFFER_RETURNED_TO_DRAFT", changed: false };
        case "PROCEED_PUBLISH":
          break;
        case "PROCEED_ARCHIVE":
          break;
        case "PROCEED_RETURN_TO_DRAFT":
          break;
      }
      ...
      if (decision.kind === "PROCEED_RETURN_TO_DRAFT") {
        await tx.update(offers).set({ publicationStatus: "draft", updatedAt: sql`CURRENT_TIMESTAMP` }).where(eq(offers.id, offer.id));
        return { ok: true, code: "OFFER_RETURNED_TO_DRAFT", changed: true };
      }
