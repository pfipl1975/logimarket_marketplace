...
export type AdminOfferStatusFilter = "draft" | "pending_review" | "published" | "archived" | "hidden" | "deleted";
...
function parseStatus(val: unknown): AdminOfferStatusFilter | null {
  const str = getSingleString(val);
  if (str === "draft" || str === "pending_review" || str === "published" || str === "archived" || str === "hidden" || str === "deleted") return str;
  return null;
}
