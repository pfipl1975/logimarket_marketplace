...
export async function getAdminOfferDetailReadModel(
  ...
  const categoryConfig = await getCategoryAttributeConfigurationFromDb(db, offer.categoryId, locale, false, false);
  ...
  const allDefinitions = attrIds.size > 0 
    ? await db.select().from(schema.attributeDefinitions).where(inArray(schema.attributeDefinitions.id, Array.from(attrIds)))
    : [];
  ...
  for (const attrId of attrIds) {
    ...
    if (dataType === "multi_enum") {
      ...
      if (val) {
        if (dataType === "text" && val.valueText !== null) values.push(val.valueText);
        else if (dataType === "number" && val.valueNumber !== null) values.push(val.valueNumber);
        else if (dataType === "boolean" && val.valueBoolean !== null) values.push(val.valueBoolean ? "true" : "false");
        ...
        else if (dataType === "year" && val.valueYear !== null) values.push(val.valueYear.toString());
  ...
  // Sort by category config sortOrder if available, then by stableKey
  ...
  return {
    ...
    data: {
      ...
      technicalAttributes: offer.technicalAttributes as Record<string, any>,
