...
export async function getAdminDashboardReadModel(db: NodePgDatabase<typeof schema>): Promise<AdminDashboardReadResult> {
  const [
    ...
    db
      .select({
        ...
        pendingReview: sql<number>`count(*) filter (where ${schema.offers.publicationStatus} = 'pending_review')`,
  ...
  const parsedOffers = {
    ...
    pendingReview: Number(offersData.pendingReview),
...
export function validateDashboardInvariants(
  ...
  const storedOffersTotal = parsedOffers.draft + parsedOffers.pendingReview + parsedOffers.published + parsedOffers.hidden + parsedOffers.archived + parsedOffers.deleted;
