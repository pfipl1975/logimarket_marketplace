...
export type OfferPublicationStatus = "draft" | "pending_review" | "published" | "hidden" | "archived" | "deleted";
...
export const offers = pgTable("offers", {
  ...
  check("offers_publication_status_check", sql`((publication_status)::text = ANY ((ARRAY['draft'::character varying, 'pending_review'::character varying, 'published'::character varying, 'hidden'::character varying, 'archived'::character varying, 'deleted'::character varying])::text[]))`)
]);
...
export const sellerTaxIdentifiers = pgTable("seller_tax_identifiers", {
  ...
  canonicalIdentityClass: varchar("canonical_identity_class", { length: 50 }).notNull(),
  canonicalIdentifierValue: varchar("canonical_identifier_value", { length: 100 }).notNull(),
  verificationStatus: varchar("verification_status", { length: 30 }).notNull().default("unverified"),
  verifiedAt: timestamp("verified_at", { withTimezone: true }),
  verificationSource: varchar("verification_source", { length: 100 }),
  verificationReference: text("verification_reference"),
...
// -----------------------------------------------------------------------------
// Buyer Organization and Internal Trust Foundation
// -----------------------------------------------------------------------------

export type BuyerOrganizationVerificationStatus = "pending" | "verified" | "rejected" | "revoked";
export type BuyerOrganizationMembershipRole = "organization_admin" | "authorized_buyer";
export type BuyerOrganizationMembershipStatus = "active" | "inactive" | "revoked";
export type BuyerOrganizationVerificationEventType = "verified" | "rejected" | "revoked" | "invalidated";
export type BuyerOrganizationVerificationActorType = "admin" | "buyer_user" | "system" | "external_adapter";
export type BuyerOrganizationVerificationSourceType = "admin_manual" | "buyer_change" | "system_rule" | "external_adapter";

export const buyerOrganizations = pgTable("buyer_organizations", {
  id: bigserial("id", { mode: "number" }).primaryKey(),
  legalName: varchar("legal_name", { length: 255 }).notNull(),
  jurisdictionCountry: varchar("jurisdiction_country", { length: 2 }).notNull(),
  verificationStatus: varchar("verification_status", { length: 20 })
    ...
    .notNull()
    .default("pending"),
  currentVerificationEventId: bigint("current_verification_event_id", { mode: "number" }),
  verifiedAt: timestamp("verified_at", { withTimezone: true }),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp("updated_at", { withTimezone: true }),
}, (t) => [
  check("chk_buyer_organizations_country", sql`jurisdiction_country ~ '^[A-Z]{2}$'`),
  check("chk_buyer_organizations_verification_status", sql`verification_status IN ('pending', 'verified', 'rejected', 'revoked')`),
  check("chk_buyer_organizations_verification_consistency", sql`
  ...
  `),
  index("idx_buyer_organizations_verification_status").on(t.verificationStatus),
]);

export const buyerOrganizationMemberships = pgTable("buyer_organization_memberships", {
  id: bigserial("id", { mode: "number" }).primaryKey(),
  authUserId: uuid("auth_user_id").notNull(),
  buyerOrganizationId: bigint("buyer_organization_id", { mode: "number" }).notNull(),
  membershipRole: varchar("membership_role", { length: 30 })
  ...
  foreignKey({
    name: "buyer_organization_memberships_organization_fk",
    columns: [t.buyerOrganizationId],
    foreignColumns: [buyerOrganizations.id],
  }).onDelete("restrict").onUpdate("no action"),
  unique("uq_buyer_organization_membership").on(t.authUserId, t.buyerOrganizationId),
  check("chk_buyer_organization_membership_role", sql`membership_role IN ('organization_admin', 'authorized_buyer')`),
  check("chk_buyer_organization_membership_status", sql`membership_status IN ('active', 'inactive', 'revoked')`),
  check("chk_buyer_organization_membership_consistency", sql`
    (membership_status = 'active' AND ended_at IS NULL)
    OR (membership_status IN ('inactive', 'revoked') AND ended_at IS NOT NULL)
  `),
  index("idx_buyer_memberships_auth_status").on(t.authUserId, t.membershipStatus),
  index("idx_buyer_memberships_org_status").on(t.buyerOrganizationId, t.membershipStatus),
]);

export const buyerTaxIdentifiers = pgTable("buyer_tax_identifiers", {
  id: bigserial("id", { mode: "number" }).primaryKey(),
  buyerOrganizationId: bigint("buyer_organization_id", { mode: "number" }).notNull(),
  identifierType: varchar("identifier_type", { length: 50 }).notNull(),
  identifierValue: varchar("identifier_value", { length: 100 }).notNull(),
  ...
  canonicalIdentityClass: varchar("canonical_identity_class", { length: 50 }).notNull(),
  canonicalIdentifierValue: varchar("canonical_identifier_value", { length: 100 }).notNull(),
  trustedByVerificationEventId: bigint("trusted_by_verification_event_id", { mode: "number" }),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  retiredAt: timestamp("retired_at", { withTimezone: true }),
}, (t) => [
  foreignKey({
    name: "buyer_tax_identifiers_organization_fk",
    columns: [t.buyerOrganizationId],
    foreignColumns: [buyerOrganizations.id],
  }).onDelete("restrict").onUpdate("no action"),
  unique("uq_buyer_tax_identifier_owner").on(t.id, t.buyerOrganizationId),
  unique("uq_buyer_tax_identifier_identity").on(t.buyerOrganizationId, t.identifierType, t.countryCode, t.identifierValue),
  check("chk_buyer_tax_identifier_type", sql`identifier_type IN ('tax_id', 'vat_id')`),
  check("chk_buyer_tax_identifier_country", sql`country_code ~ '^[A-Z]{2}$'`),
...
export const buyerRegistryIdentifiers = pgTable("buyer_registry_identifiers", {
  ...
  foreignKey({
    name: "buyer_registry_identifiers_organization_fk",
    columns: [t.buyerOrganizationId],
    foreignColumns: [buyerOrganizations.id],
  }).onDelete("restrict").onUpdate("no action"),
  unique("uq_buyer_registry_identifier_owner").on(t.id, t.buyerOrganizationId),
  unique("uq_buyer_registry_identifier_identity").on(t.buyerOrganizationId, t.registryType, t.jurisdictionCountry, t.registryValue),
  check("chk_buyer_registry_identifier_type", sql`registry_type IN ('commercial_register', 'statistical_id')`),
  check("chk_buyer_registry_identifier_country", sql`jurisdiction_country ~ '^[A-Z]{2}$'`),
  check("chk_buyer_registry_identifier_value", sql`length(btrim(registry_value)) > 0`),
  check("chk_buyer_registry_identifier_trust_retirement", sql`trusted_by_verification_event_id IS NULL OR retired_at IS NULL`),
  index("idx_buyer_registry_identifiers_active_org").on(t.buyerOrganizationId).where(sql`retired_at IS NULL`),
  index("idx_buyer_registry_identifiers_active_value").on(t.registryType, t.jurisdictionCountry, t.registryValue).where(sql`retired_at IS NULL`),
]);

export const buyerOrganizationVerificationEvents = pgTable("buyer_organization_verification_events", {
  id: bigserial("id", { mode: "number" }).primaryKey(),
  buyerOrganizationId: bigint("buyer_organization_id", { mode: "number" }).notNull(),
  ...
  sourceName: varchar("source_name", { length: 100 }),
  sourceReference: text("source_reference"),
  verificationMethod: varchar("verification_method", { length: 100 }).notNull(),
  reasonCode: varchar("reason_code", { length: 100 }),
  previousVerificationStatus: varchar("previous_verification_status", { length: 20 })
    .$type<BuyerOrganizationVerificationStatus>()
    .notNull(),
  legalNameSnapshot: varchar("legal_name_snapshot", { length: 255 }).notNull(),
  jurisdictionCountrySnapshot: varchar("jurisdiction_country_snapshot", { length: 2 }).notNull(),
  taxIdentifierId: bigint("tax_identifier_id", { mode: "number" }),
  taxIdentifierTypeSnapshot: varchar("tax_identifier_type_snapshot", { length: 50 }),
  taxIdentifierValueSnapshot: varchar("tax_identifier_value_snapshot", { length: 100 }),
  taxCountryCodeSnapshot: varchar("tax_country_code_snapshot", { length: 2 }),
  registryIdentifierId: bigint("registry_identifier_id", { mode: "number" }),
  registryTypeSnapshot: varchar("registry_type_snapshot", { length: 50 }),
  registryValueSnapshot: varchar("registry_value_snapshot", { length: 100 }),
  registryCountryCodeSnapshot: varchar("registry_country_code_snapshot", { length: 2 }),
  occurredAt: timestamp("occurred_at", { withTimezone: true }).notNull().defaultNow(),
}, (t) => [
  foreignKey({
    name: "buyer_verification_events_organization_fk",
    columns: [t.buyerOrganizationId],
    foreignColumns: [buyerOrganizations.id],
  }).onDelete("restrict").onUpdate("no action"),
  unique("uq_buyer_verification_event_owner").on(t.id, t.buyerOrganizationId),
  check("chk_buyer_verification_event_type", sql`event_type IN ('verified', 'rejected', 'revoked', 'invalidated')`),
  check("chk_buyer_verification_outcome_status", sql`outcome_status IN ('pending', 'verified', 'rejected', 'revoked')`),
  check("chk_buyer_verification_previous_status", sql`previous_verification_status IN ('pending', 'verified', 'rejected', 'revoked')`),
  check("chk_buyer_verification_event_outcome", sql`
  ...
  `),
  check("chk_buyer_verification_actor_type", sql`actor_type IN ('admin', 'buyer_user', 'system', 'external_adapter')`),
  check("chk_buyer_verification_source_type", sql`source_type IN ('admin_manual', 'buyer_change', 'system_rule', 'external_adapter')`),
  check("chk_buyer_verification_actor", sql`
  ...
  `),
  check("chk_buyer_verification_source_actor", sql`
  ...
  `),
  check("chk_buyer_verification_tax_snapshot", sql`
  ...
  `),
  check("chk_buyer_verification_registry_snapshot", sql`
  ...
  `),
  check("chk_buyer_verification_identifier_present", sql`tax_identifier_id IS NOT NULL OR registry_identifier_id IS NOT NULL`),
  check("chk_buyer_verification_evidence", sql`
    event_type <> 'verified'
    OR (length(btrim(source_name)) > 0 AND length(btrim(source_reference)) > 0)
  `),
  check("chk_buyer_verification_reason", sql`
    event_type = 'verified'
    OR length(btrim(reason_code)) > 0
  `),
  check("chk_buyer_verification_country", sql`jurisdiction_country_snapshot ~ '^[A-Z]{2}$'`),
  index("idx_buyer_verification_events_org_time").on(t.buyerOrganizationId, t.occurredAt),
  index("idx_buyer_verification_events_type_time").on(t.eventType, t.occurredAt),
  index("idx_buyer_verification_events_tax").on(t.taxIdentifierId),
  index("idx_buyer_verification_events_registry").on(t.registryIdentifierId),
]);
...
export const notificationOutboxEvents = pgTable("notification_outbox_events", {
  ...
  check("chk_outbox_event_type", sql`event_type IN ('seller_order.routed_to_seller', 'seller_order.accepted_for_buyer', 'seller_order.rejected_for_buyer', 'seller_order.expired_for_seller', 'seller_order.expired_for_buyer')`),
