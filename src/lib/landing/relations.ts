...
} as const satisfies Record<string, IntentRelations>;

const groupIntentRelations = {
...
export function resolveCategorySolutionLinks({
  ...
  for (const slug of candidates) {
    const intents =
      categoryIntentOverrides[slug as keyof typeof categoryIntentOverrides] ??
      groupIntentRelations[slug as keyof typeof groupIntentRelations] ??
      sectionIntentRelations[slug as keyof typeof sectionIntentRelations];
...
export function resolveGlossarySolutionLinks(
  ...
  const intents = glossaryIntentRelations[glossarySlug as keyof typeof glossaryIntentRelations];
  return intents ? toLandingLinks(intents, locale) : [];
}

export const categorySolutionRelationSlugs = {
  sections: Object.keys(sectionIntentRelations),
  groups: Object.keys(groupIntentRelations),
  categoryOverrides: Object.keys(categoryIntentOverrides),
};

export const glossarySolutionRelationSlugs = Object.keys(glossaryIntentRelations);
