...
test("Dashboard Invariants Logic", () => {
  const baseOffers = { total: 0, draft: 0, pendingReview: 0, published: 0, hidden: 0, archived: 0, deleted: 0 };
  ...
  assert.doesNotThrow(() => validateDashboardInvariants(
    { ...baseOffers, total: 1, pendingReview: 1 },
    basePartners,
    baseEligibility,
    baseRfq,
  ));
  assert.throws(() => validateDashboardInvariants(
    { ...baseOffers, total: 1 },
    basePartners,
    baseEligibility,
    baseRfq,
  ), /unknown offer status present/);
