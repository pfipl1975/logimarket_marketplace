...
describe("UI Contract Tests", () => {
  test("AdminOffersTable has NO lifecycle action", () => {
    ...
    assert.ok(tableSrc.includes("pending_review"), "Table must include pending-review label");
  ...
  test("AdminOfferDetailPage renders workspace and specific states", () => {
    ...
    assert.ok(
      detailSrc.includes("offer.publicationStatus === \"draft\" || offer.publicationStatus === \"pending_review\" || offer.publicationStatus === \"published\""),
      "AdminOfferLifecycleAction must render for draft, pending review, or published"
    );
    assert.ok(detailSrc.includes('case "pending_review": return dict.statusPendingReview;'), "Pending review must be a known detail status");
  ...
  test("AdminDashboardPage respects PII boundaries and layout", () => {
    ...
    assert.ok(dashboardSrc.includes("href={`${routes.offers}?status=pending_review`}"), "Must contain pending-review filter link");
