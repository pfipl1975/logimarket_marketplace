...
describe("Offer Publication Parser", () => {
  ...
  test("accepts pending_review approval and return-to-draft forms", () => {
    assert.deepEqual(parseAdminOfferPublicationInput({
      offerId: "7",
...
describe("Offer Publication State Machine", () => {
  ...
  test("PROCEED: pending_review -> published", () => {
    assert.deepEqual(evaluateOfferPublicationTransition("pending_review", "pending_review", "published"), { kind: "PROCEED_PUBLISH" });
  });

  test("PROCEED: pending_review -> draft", () => {
    assert.deepEqual(evaluateOfferPublicationTransition("pending_review", "pending_review", "draft"), { kind: "PROCEED_RETURN_TO_DRAFT" });
  });
  ...
  test("Parser allows every known expected status and only lifecycle command targets", () => {
    ...
    assert.ok(parseAdminOfferPublicationInput({ offerId: "123", expectedStatus: "pending_review", targetStatus: "draft" }));
