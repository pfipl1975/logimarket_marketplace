...
import { projectAttributeValues } from "../../src/lib/catalog/offer-attributes-read-model";
...
function makeMockDb(offer: unknown, partner?: unknown, category?: unknown) {
  ...
  const q = {
    ...
    limit() {
      ...
      if (this._table === schema.offers) {
        return Promise.resolve(
          offer
            ? [{ offer, partner: partner ?? null, category: category ?? null }]
            : [],
        );
      }
      return this;
    ...
    then(resolve: (v: unknown) => void) {
      ...
      else if (this._table === schema.offerAttributeOptionValues)
        resolve(state.oaovRows);
      else if (this._table === schema.attributeDefinitions)
        resolve(state.attrDefs);
      else resolve([]);
    },
  };
  return q;
...
describe("resolveCanonicalOfferModel", () => {
  ...
  for (const [model, conversion, expected] of cases) {
    it(`${model} + ${conversion} => ${expected}`, () => {
      assert.strictEqual(
        resolveCanonicalOfferModel(model, conversion),
        expected,
      );
    });
  }
  ...
  it("ISO date => date part only (YYYY-MM-DD)", () => {
    const result = projectAttributeValues(
      ...
      [
        makeOav({
          attributeId: 1,
          valueDate: new Date("2024-06-15T00:00:00Z"),
        }),
      ],
      [],
    );
...
describe("projectAttributeValues – enum with localized label", () => {
  ...
  it("optionId fallback: returns [Option N] string with correct optionId (no template literal regression)", () => {
    ...
    assert.ok(
      !result[0].includes("\\$"),
      "Must not contain escaped template literal",
    );
    assert.ok(
      !result[0].includes("${"),
      "Must not contain raw template literal syntax",
    );
  });
});
...
describe("projectAttributeValues – multi_enum with deterministic ordering", () => {
  ...
  it("multi_enum optionId fallback: uses [Option N] with correct ID (no template literal regression)", () => {
    ...
    assert.ok(
      result.includes("[Option 42]"),
      `Expected [Option 42], got: ${JSON.stringify(result)}`,
    );
    assert.ok(
      result.includes("[Option 77]"),
      `Expected [Option 77], got: ${JSON.stringify(result)}`,
    );
  ...
  it("multi_enum with no config: sorted by optionId as fallback", () => {
    const oaovRows = [makeOaov(20, 50), makeOaov(20, 10), makeOaov(20, 30)];
    const result = projectAttributeValues(
      20,
      "multi_enum",
      undefined,
...
describe("unknown_ stableKey template literal regression", () => {
  it("produces unknown_N with correct N, not escaped literal", async () => {
    ...
    const { isCanonicalPositiveInteger } =
      await import("../../src/lib/admin/offers-query");
...
describe("getAdminOfferDetail – auth contract", () => {
  it("actions.ts calls requireAdmin() inside getAdminOfferDetail before read model", () => {
    ...
    assert.ok(
      afterFn.includes("requireAdmin"),
      "requireAdmin must be called inside getAdminOfferDetail",
    );
    ...
    assert.ok(
      requireAdminIdx < readModelIdx,
      "requireAdmin() must come before getAdminOfferDetailReadModel()",
    );
  });
  ...
  it("getAdminOfferDetail has sanitized error logging (no error.message exposure)", () => {
    ...
    assert.ok(
      !fnBody.includes("error instanceof Error"),
      "Must not use error instanceof Error for logging",
    );
  });
});
...
describe("Route contract – PL /admin/oferty/[id]", () => {
  ...
  it("uses AdminOfferDetailPage with defaultLocale", () => {
    ...
    assert.ok(
      src.includes("AdminOfferDetailPage"),
      "must render AdminOfferDetailPage",
    );
...
describe("Route contract – localized /[locale]/admin/offers/[id]", () => {
  ...
  it("calls notFound() for invalid locale", () => {
    ...
    assert.ok(
      ...
      "must call notFound() for invalid locale",
    );
    assert.ok(
      src.includes("isLocale"),
      "must check locale validity with isLocale",
    );
  });
});
...
describe("AdminOffersTable – list -> detail link contract", () => {
  it("getDetailUrl produces correct PL path", () => {
    ...
    const id = 55;
    const url =
      locale === "pl" ? `/admin/oferty/${id}` : `/${locale}/admin/offers/${id}`;
...
describe("getAdminOfferDetailReadModel – successful projection", () => {
  it("preserves pending_review as a known internal status", async () => {
    const partner = { id: 7, companyName: "Test Partner Sp. z o.o." };
    const category = { id: 3, name: "Test Category", slug: "test-category" };
    const db = makeMockDb(makeOkOffer({ publicationStatus: "pending_review" }), partner, category);

    const result = await getAdminOfferDetailReadModel(db as never, "42", "pl");
    assert.ok(result.ok);
    assert.strictEqual(result.data.publicationStatus, "pending_review");
    assert.strictEqual(result.data.publicPreviewAllowed, false);
  });
  ...
  it("projects raw offer fields without mutation", async () => {
    ...
    db._state.oavRows.push(
      makeOav({ attributeId: 103, valueText: "orphan value" }),
    );
    ...
    assert.deepStrictEqual(d.technicalAttributes, {
      width: 100,
      material: "steel",
    });
    ...
    const byAttrId = new Map(
      d.relationalAttributes.map((a) => [a.attributeId, a]),
    );
