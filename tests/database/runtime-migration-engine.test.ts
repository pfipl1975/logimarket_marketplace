...
const exactJournalEntries = [
  ...
  { tag: "fake_tag_0010", when: 1785594500000 },
  { tag: "fake_tag_0011", when: 1785595000000 },
...
function metadataRouter(options?: MetadataOptions): Router {
  ...
  return (t) => {
    ...
    if (t.includes("pg_proc")) {
      if (t.includes("prevent_buyer_organization_verification_events_mutation") && !fp["buyer_organizations"]) {
        return { rows: [] };
      }
    ...
    const base =
      ...
      : state === "PREVIOUS"
        ? metadataRouter({
            ...
            journalRows: prevFakeRead().map(({ hash, folderMillis }) => ({ hash, created_at: String(folderMillis) })),
          })
        : state === "BASELINE"
...
test("TARGET: EXACT_EXISTING_POST_0015 for the historical POST_0015 fingerprint", () => {
  const result = classifyRuntimeTarget(FINAL_POST_0015_PRODUCTION_FINGERPRINT, Object.keys(FINAL_POST_0015_PRODUCTION_FINGERPRINT));
...
for (const { expectedPreState, actualState, fingerprint } of [
  ...
  { expectedPreState: "MIGRATABLE_POST_0004", actualState: "EXACT_EXISTING_POST_0015", fingerprint: FINAL_POST_0015_PRODUCTION_FINGERPRINT },
...
test("RUNNER_POST0014_STAYS_POST0014_BLOCK: post-check fails if 0015 is not applied", async () => {
  ...
  // PREVIOUS is the exact post-0013 predecessor state.
  ...
  await assert.rejects(
    async () => runMigrations(
      ...
      prevFakeRead as never,
      prevFakeReadFn as never,
