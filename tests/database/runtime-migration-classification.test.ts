...
test("TARGET_EXACT_POST_0017_EXPLICIT_FINGERPRINT", () => {
  const actual = buildSide(FINAL_POST_0017_PRODUCTION_FINGERPRINT);
  const result = classifyRuntimeTarget(actual, Object.keys(actual), POST_0009_SECURITY_CONTRACT);
  assert.strictEqual(result.state, "EXACT_EXISTING_POST_0017");
});

test("TARGET_EXACT_POST_0018_EXPLICIT_FINGERPRINT", () => {
  const actual = buildSide(FINAL_POST_0018_PRODUCTION_FINGERPRINT);
  const result = classifyRuntimeTarget(actual, Object.keys(actual), POST_0018_SECURITY_CONTRACT);
  assert.strictEqual(result.state, "EXACT_EXISTING_POST_0018");
});
