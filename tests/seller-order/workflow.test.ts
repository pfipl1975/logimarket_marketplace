...
import {
  ...
  evaluateSellerOrderFulfillmentTransition,
...
test("P1. canonical acceptance advances through fulfillment exactly once per state", () => {
  const decision = {
    decisionStatus: "seller_accepted",
    expiresAt,
    acceptedAt: beforeDeadline,
    resolvedAt: beforeDeadline,
    decidedByAuthUserId: "u1",
...
test("P1a. pending and rejected orders cannot skip into fulfillment", () => {
...
test("P1b. fulfillment is fail-closed without canonical acceptance evidence", () => {
...
test("P1c. fulfilled orders cannot regress or repeat the final transition", () => {
  const decision = {
    decisionStatus: "seller_accepted",
    expiresAt,
    acceptedAt: beforeDeadline,
