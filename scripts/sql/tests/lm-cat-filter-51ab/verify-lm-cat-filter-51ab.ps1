...
try {
  ...
  # T19 — source offers unchanged
  # T20 — configuration rows unchanged
  Invoke-Case 'T09_T20' {
    $db = New-Database 'T09'
  ...
  Invoke-Case 'T21_T24' {
    ...
    # Sequence and row IDs/timestamps must be identical
    Assert-That ($seqBefore -eq $seqAfter) "idempotency failed: sequence advanced from $seqBefore to $seqAfter"
    Assert-That ($rowsBefore -eq $rowsAfter) "idempotency failed: rows state changed after second run"
  }
  ...
  Invoke-Case 'T34_T38' {
    ...
    # Verify exact 10 pilot rows are gone
    $oavCount = Get-Scalar $db "SELECT count(*) FROM public.offer_attribute_values WHERE offer_id IN (5, 6)"
    Assert-That ($oavCount -eq '0') "rollback left offer values: $oavCount"
    
    # Verify config still has 88 rows
    $ad = Get-Scalar $db "SELECT count(*) FROM public.attribute_definitions"
    $adt = Get-Scalar $db "SELECT count(*) FROM public.attribute_definition_translations"
    $cov = Get-Scalar $db "SELECT count(*) FROM public.controlled_option_values"
    $covt = Get-Scalar $db "SELECT count(*) FROM public.controlled_option_value_translations"
  ...
  # T47 — Git diff check (run by executor run-all)
  ...
  Assert-That ($results.Count -eq 24) "expected 24 executed cases/groups, got $($results.Count)"
