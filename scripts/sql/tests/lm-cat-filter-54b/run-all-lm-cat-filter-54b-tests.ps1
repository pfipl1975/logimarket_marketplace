...
# LM-CAT-FILTER-54B — Local Test execution wrapper
$root = Resolve-Path (Join-Path $PSScriptRoot '..\..\..\..')
$runner = Join-Path $PSScriptRoot 'verify-lm-cat-filter-54b.ps1'
...
# Check for orphaned postgres processes from our run prefix (lm54b)
Get-Process postgres -ErrorAction SilentlyContinue | Where-Object { $_.Path -match 'lm54b' } | ForEach-Object {
...
# Check for orphaned postgres processes after the run
$noOrphans = $false
$orphanedProcs = Get-Process postgres -ErrorAction SilentlyContinue | Where-Object { $_.Path -match 'lm54b' }
...
# Check for deleted temporary folders
$noRemainingFolders = $false
$remainingFolders = Get-ChildItem $env:TEMP -Directory -Filter "lm54b-filter-*" -ErrorAction SilentlyContinue
if ($remainingFolders.Count -eq 0) {
  $noRemainingFolders = $true
} else {
...
# We only allow scripts/sql/production/ and scripts/sql/tests/lm-cat-filter-54b/
$statusLines = $gitStatus -split "`n" | Where-Object { $_.Trim() }
foreach ($line in $statusLines) {
  $file = ($line.Trim() -split '\s+')[1]
  if ($file -notmatch '^scripts/sql/production/' -and $file -notmatch '^scripts/sql/tests/lm-cat-filter-54b/') {
...
$cleanupPassed = $false
if ($output -match 'LM54B_CLEANUP=PASS') {
  $cleanupPassed = $true
}

$allTestsPassed = $runSucceeded -and $gitDiffCheck -and $forbiddenScope -and $cleanupPassed -and $noOrphans -and $noRemainingFolders -and ($missingIds.Count -eq 0) -and ($unknownIds.Count -eq 0) -and ($duplicateCount -eq 0)
...
if ($allTestsPassed) {
  Write-Output "LM54B_TESTS=$($uniqueIds.Count)/$($expectedIds.Count) PASS"
  Write-Output "LM54B_FORWARD=PASS"
  Write-Output "LM54B_IDEMPOTENCY=PASS"
  Write-Output "LM54B_CONFLICT_GUARDS=PASS"
  Write-Output "LM54B_ROLLBACK=PASS"
  Write-Output "LM54B_ROLLBACK_DRIFT_GUARD=PASS"
  Write-Output "LM54B_CLEANUP=PASS"
  Write-Output "LM54B_ALL_TESTS=PASS"
  exit 0
} else {
  Write-Output "LM54B_ALL_TESTS=FAIL"
  exit 1
}
