...
BEGIN
  ...
  IF v_orig_category_id = 25 THEN
    -- Stan ALREADY_APPLIED
    RAISE NOTICE 'LM55A apply: ALREADY_APPLIED. Offer 8 is already in category 25.';
  ELSIF v_orig_category_id = 21 THEN
    -- Wykonaj UPDATE
  ...
  ELSE
    ...
    RAISE EXCEPTION 'LM55A apply: DRIFT_DETECTED. Offer 8 is in category %, expected 21 or 25', v_orig_category_id USING ERRCODE = 'check_violation';
  END IF;
  ...
  IF v_post_slug_21 IS DISTINCT FROM 'wozki-widlowe-elektryczne' THEN
    RAISE EXCEPTION 'LM55A apply postcheck: source category slug mutated' USING ERRCODE = 'check_violation';
  END IF;
  ...
  IF v_post_slug_25 IS DISTINCT FROM 'elektryczne-wozki-paletowe' THEN
    RAISE EXCEPTION 'LM55A apply postcheck: target category slug mutated' USING ERRCODE = 'check_violation';
  END IF;
  ...
  IF v_post_offer_1_cat IS DISTINCT FROM 21 THEN
    RAISE EXCEPTION 'LM55A apply postcheck: offer 1 category changed to %', v_post_offer_1_cat USING ERRCODE = 'check_violation';
  END IF;
END $$;
