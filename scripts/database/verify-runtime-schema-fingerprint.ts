...
export function classifyRuntimeTarget(
  actual: Record<string, TableFingerprintSide>,
  allPublicTables: string[],
  security?: RuntimeSecurityContract
): RuntimeTargetResult {
  const publicTableCount = allPublicTables.length;
  const effectiveSecurity = security ?? (allPublicTables.includes("buyer_organizations") ? POST_0018_SECURITY_CONTRACT : POST_0009_SECURITY_CONTRACT);
  ...
  const matchPost0017 = compareRuntimeFingerprint(actual, allPublicTables, FINAL_POST_0017_PRODUCTION_FINGERPRINT);

  if (matchPost0017.isExactMatch) {
    if (JSON.stringify(effectiveSecurity) === JSON.stringify(POST_0009_SECURITY_CONTRACT)) {
      return { state: "EXACT_EXISTING_POST_0017", publicTableCount, differences: [] };
    }
    return { state: "PARTIAL_OR_DRIFTED", publicTableCount, differences: ["Function security configuration drifted"] };
  }

  const matchPost0016 = compareRuntimeFingerprint(actual, allPublicTables, FINAL_POST_0016_PRODUCTION_FINGERPRINT);

  if (matchPost0016.isExactMatch) {
    if (JSON.stringify(effectiveSecurity) === JSON.stringify(POST_0009_SECURITY_CONTRACT)) {
      return { state: "EXACT_EXISTING_POST_0016", publicTableCount, differences: [] };
    }
    return { state: "PARTIAL_OR_DRIFTED", publicTableCount, differences: ["Function security configuration drifted"] };
  }

  const matchPost0015 = compareRuntimeFingerprint(actual, allPublicTables, FINAL_POST_0015_PRODUCTION_FINGERPRINT);

  if (matchPost0015.isExactMatch) {
    if (JSON.stringify(effectiveSecurity) === JSON.stringify(POST_0009_SECURITY_CONTRACT)) {
      return { state: "EXACT_EXISTING_POST_0015", publicTableCount, differences: [] };
    }
    return { state: "PARTIAL_OR_DRIFTED", publicTableCount, differences: ["Function security configuration drifted"] };
  }

  const matchPost0014 = compareRuntimeFingerprint(actual, allPublicTables, FINAL_POST_0014_PRODUCTION_FINGERPRINT);

  if (matchPost0014.isExactMatch) {
    if (JSON.stringify(effectiveSecurity) === JSON.stringify(POST_0009_SECURITY_CONTRACT)) {
  ...
  if (matchPost0008.isExactMatch) {
    if (JSON.stringify(effectiveSecurity) === JSON.stringify(POST_0008_SECURITY_CONTRACT)) {
      return { state: "EXACT_EXISTING_POST_0008", publicTableCount, differences: [] };
    }
    if (JSON.stringify(effectiveSecurity) === JSON.stringify(PRE_0008_SECURITY_CONTRACT)) {
...
export async function fetchLiveSchemaMetadata(
  ...
  const funcResultBuyerVerification = await q.query(`
    SELECT p.proconfig
    FROM pg_proc p
    JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public' AND p.proname = 'prevent_buyer_organization_verification_events_mutation'
  `);
  let searchPathBuyerVerification: string[] | null = null;
  if (funcResultBuyerVerification.rows.length === 1) {
    searchPathBuyerVerification = (funcResultBuyerVerification.rows[0] as { proconfig: string[] | null }).proconfig;
  } else if (funcResultBuyerVerification.rows.length > 1) {
    searchPathBuyerVerification = ['DUPLICATE_FUNCTION'];
  }
  ...
  return {
    ...
    security: {
      ...
      preventBuyerOrganizationVerificationEventsMutationSearchPath: searchPathBuyerVerification,
    },
  };
}
