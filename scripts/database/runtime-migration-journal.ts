...
export function validateAppliedMigrationPrefix(
  ...
  } else if (schemaClassificationState === "EXACT_EXISTING_POST_0016") {
    if (appliedRows.length !== 17) {
      throw new Error(`RUNNER: BLOCKED. Journal states do not match exact canonical 0000 (schema is POST_0016 but journal has ${appliedRows.length} rows)`);
    }
  ...
  } else if (schemaClassificationState === "EXACT_EXISTING_POST_0017") {
    if (appliedRows.length !== 18) {
      throw new Error(`RUNNER: BLOCKED. Journal states do not match exact canonical 0000 (schema is POST_0017 but journal has ${appliedRows.length} rows)`);
    }
  } else if (schemaClassificationState === "EXACT_EXISTING_POST_0018") {
