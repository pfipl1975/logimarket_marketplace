...
$root = Join-Path $env:TEMP ("lm47-" + [Guid]::NewGuid().ToString('N')); $data = Join-Path $root 'data'; $log = Join-Path $root 'postgres.log'; $dbName = 'lm47'
...
try {
  ...
  npx.cmd --no-install tsx scripts/lm47-integration.ts; if ($LASTEXITCODE -ne 0) { throw 'LM47 integration failed' }
} finally {
